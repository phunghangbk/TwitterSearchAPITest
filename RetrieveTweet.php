...
class RetrieveTweet
{
    ...
    {
    	$this->bdd = new DatabasePDO();
    }

    public function getTweets($access_token, $access_token_secret, $consumer_key, $consumer_secret, $request)
    {
        $errors = [];
        $startDate = '';
        $endDate = '';
        ...
        try {
            ...
            if (count($records) > 0) {
                if (! $tweet->insert($records)) {
                	$request['status'] = self::ERROR_MESSAGE;
                	$searchinfo->savesearchinfo($request);
            ...
            $request['status'] = self::SUCCESS_MESSAGE;
            $searchinfo->savesearchinfo($request);
            return json_encode([
                'status' => self::SUCCESS_STATUS,
     ...
     * @return [json]   $tweets
     */
    private function retrieveTweets($twitter, $parameters, $url = 'https://api.twitter.com/1.1/search/tweets.json')
    {
        $unavailableCnt = 0;
        while(1) {
    ...
    private function sort($tweets) {
        if (isset($tweets->statuses)) {
        	$tweets->statuses = array_unique($tweets->statuses, SORT_REGULAR);
    ...
    private function createParameters($startDate, $startTime, $endDate, $endTime, $keyword)
    {
        return '?q=' . $keyword . 
            ...
            ' -filter:retweets and -filter:replies&count=100&tweet_mode=extended&result_type=popular';
    }
